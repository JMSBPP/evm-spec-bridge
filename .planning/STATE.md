...
- **NEW (Phase 3 input)**: array signedness is NOT observable on the wire — `int256[]` and `uint256[]` have identical ABI layouts, so a wrong-branch decode CANNOT revert (measured by `gams-evm-transport`). Signedness must be asserted by the schema. Lands on Phase 8/9 codegen.
